...
dashboardRouter.get("/stats", async (req, res): Promise<void> => {
  ...
  const [closedSessionsRow] = await db
    .select({ count: count() })
    .from(sessionsTable)
    .where(eq(sessionsTable.status, "closed"));
  ...
  res.json({
    ...
    closedSessions: Number(closedSessionsRow?.count ?? 0),
...
dashboardRouter.get("/threat-breakdown", async (req, res): Promise<void> => {
...
dashboardRouter.get("/source-distribution", async (req, res): Promise<void> => {
